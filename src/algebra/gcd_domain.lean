...
@[simp] lemma normalize_eq_zero {x : α} : normalize x = 0 ↔ x = 0 :=
⟨λ hx, (associated_zero_iff_eq_zero x).1 $ hx ▸ associated_normalize, by rintro rfl; exact normalize_zero α⟩
...
dvd_antisymm_of_normalize_eq (normalize_gcd _ _) (normalize_one α) (gcd_dvd_left _ _) (one_dvd _)

@[simp] theorem gcd_one_right (a : α) : gcd a 1 = 1 :=
dvd_antisymm_of_normalize_eq (normalize_gcd _ _) (normalize_one α) (gcd_dvd_right _ _) (one_dvd _)
...
{ gcd            := λa b, int.gcd a b,
  ...
  normalize_gcd  := assume a b, normalize_nat_coe _,
